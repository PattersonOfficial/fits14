...
import {FITNESS_CATEGORY_ID, Subcategory} from '../../../../../../../models/categories/categories.model';
import {ARTICLE_TYPE, Contents, VIDEO_TYPE} from '../../../../../../../models/contents/contents.model';
import {ContentsService} from '../../contents/contents.service';

@Component({
    selector: 'app-plan-fitness',
    templateUrl: './plan_fitness.component.html',
    styleUrls: ['./plan_fitness.component.css']
})

export class PlanFitnessComponent implements OnInit {
    ...
    getPlans() {
        this._planService.getPlans({category_id: FITNESS_CATEGORY_ID}).subscribe(
            data => this.plans = data || []
        );
    }

    getContent() {
        this._contentsService.getListByFilter({category_id: FITNESS_CATEGORY_ID, type: VIDEO_TYPE}).subscribe(data => {
    ...
    getSubcategories() {
        this._planService.getSubcategoriesByCategoryId(FITNESS_CATEGORY_ID).subscribe(
    ...
    postContent(): void {
        if (this.plan.valid_days > 0 && this.plan.valid_days === this.planContentList.length && this.plan.name.trim() !== '') {
            ...
            this.plan.category_id = FITNESS_CATEGORY_ID;
