...
import {NUTRITION_CATEGORY_ID, Subcategory} from '../../../../../../../models/categories/categories.model';
import {ARTICLE_TYPE, Contents, VIDEO_TYPE} from '../../../../../../../models/contents/contents.model';
import {ContentsService} from '../../contents/contents.service';

@Component({
    selector: 'app-plan-nutrition',
    templateUrl: './plan_nutrition.component.html',
    styleUrls: ['./plan_nutrition.component.css']
})

export class PlanNutritionComponent implements OnInit {
    ...
    getPlans() {
        this._planService.getPlans({category_id: NUTRITION_CATEGORY_ID}).subscribe(
            data => this.plans = data || []
        );
    }

    getContent() {
        this._contentsService.getListByFilter({category_id: NUTRITION_CATEGORY_ID}).subscribe(data => {
    ...
    getSubcategories() {
        this._planService.getSubcategoriesByCategoryId(NUTRITION_CATEGORY_ID).subscribe(
    ...
    postContent(): void {
        if (this.plan.valid_days > 0 && this.plan.valid_days === this.planContentList.length && this.plan.name.trim() !== '') {
            ...
            this.plan.category_id = NUTRITION_CATEGORY_ID;
    ...
    onRemoveWorkoutClick(index: number) {
        return this.planContentList.splice(index, 1);
    }
